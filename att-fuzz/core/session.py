...
class FuzzSession:
    ...
    def health_check(self, timeout=None) -> str:
        ...
        if rsp.pdu[0] == int(AttOpcode.READ_RSP):
            if base.get("kind") == "value":
                return "ok" if base.get("value") == rsp.pdu[1:].hex() else "value_changed"
    ...
    def _fill_case_fields(self, result: CaseResult, pdu: bytes):
        result.opcode = pdu[0] if pdu else None
        if len(pdu) >= 3 and pdu[0] in (0x0A, 0x0C, 0x12, 0x16, 0x52):
            result.handle = pdu[1] | (pdu[2] << 8)
        if pdu[0] in (0x0C, 0x16) and len(pdu) >= 5:
            result.offset = pdu[3] | (pdu[4] << 8)
        if pdu[0] in (0x12, 0x52, 0x16) and result.handle is not None:
            hdr = 5 if pdu[0] == 0x16 else 3
            result.value_len = max(0, len(pdu) - hdr)
            result.value_hash = hashlib.sha1(pdu[hdr:]).hexdigest()[:12]
